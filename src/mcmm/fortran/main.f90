...
! 51 Franklin Street, Fifth Floor, Boston, MA 02110-1301 USA. 
      ...
      parameter ( g = 9.81d0 ) ! Gravitational acceleration (m/s^2)
      real*8, allocatable :: dUx(:), dUy(:), dUxold(:), dUyold(:)
      ! Variables for timeseries updates
      integer :: current_month
      real*8 :: current_W, current_D, current_E_rate, current_physical_ds
...
! load file sim with the characteristic parameters
      ...
      ! Load timeseries data if specified
      if (LEN_TRIM(filetimeseries) > 0) then
        call load_timeseries
      else
        n_timeseries = 0 ! Ensure count is 0 if no file loaded
      end if
...
!-----------------------------------------------------------------------
! UPDATE PARAMETERS FROM TIMESERIES (if applicable)
!-----------------------------------------------------------------------
        if (n_timeseries > 0) then
           ! Determine current month based on simulation time (dimensionless years)
           ! Assumes tt0 is the starting time in dimensionless years
           current_month = INT( (time - tt0) * 12.0d0 - 1.0d-9 ) + 1

           ! Update timeseries index, but cap at the number of records available
           current_ts_index = MIN(current_month, n_timeseries)

           ! Retrieve current month's physical values
           current_W = ts_W(current_ts_index)
           current_D = ts_D(current_ts_index)
           current_E_rate = ts_E_rate(current_ts_index)
           current_physical_ds = ts_physical_ds(current_ts_index)

           ! Update model parameters based on timeseries data
           ! 1. Update beta (width/depth ratio)
           if (current_D > 1.0d-6) then ! Avoid division by zero
              beta = current_W / (2.0d0 * current_D)
              ! Also update ds (dimensionless grain size)
              ds = current_physical_ds / current_D
           else
              write(6,*) 'Warning: Depth is near zero, keeping beta & ds constant this step.'
           end if

           ! 2. Update theta (Shields parameter)
           ! theta = (D * S) / (delta * physical_ds)
           if (delta_sg > 1.0d-6 .and. current_physical_ds > 1.0d-6) then ! Avoid division by zero
              theta = (current_D * slope) / (delta_sg * current_physical_ds)
           else
             write(6,*) 'Warning: delta_sg or physical_ds is near zero, keeping theta constant.'
           end if

           ! 3. Update Rp (particle Reynolds number)
           ! Rp = sqrt(delta_sg * g * physical_ds^3) / nu
           if (kinematic_viscosity > 1.0d-15 .and. current_physical_ds >= 0.d0) then
              Rp = sqrt( abs(delta_sg * g * current_physical_ds**3) ) / kinematic_viscosity
           else
              write(6,*) 'Warning: kinematic_viscosity or physical_ds invalid, keeping Rp constant.'
           end if

           ! 4. Update Erodibility Factors (Ef, Eb, Eo)
           ! Using base_Ef and E_scale approach: Ef = base_Ef + E_scale * E_rate
           ! Or simpler: Ef = E_scale * E_rate (if base_Ef is meant to be zero)
           ! Let's use Ef = E_scale * E_rate (assuming E_rate is the erodibility itself or needs simple scaling)
           ! User needs to adjust E_scale in .sim file accordingly.
           Ef = E_scale * current_E_rate
           Eb = Ef * factor_Eb ! Scale point bar erodibility
           Eo = Ef * factor_Eo ! Scale oxbow erodibility

           ! Ensure erodibilities don't become negative or excessively large if needed
           Ef = MAX(Ef, 1.0d-15) ! Set a minimum floor perhaps
           Eb = MAX(Eb, 1.0d-15)
           Eo = MAX(Eo, 1.0d-15)

           ! TODO (Future): Update Cf (friction coefficient) dynamically if needed.
           ! This would likely involve calling resistance_flat_bed or resistance_dune_bed
           ! with current Q, W, D, S etc., or using an empirical formula.
           ! For now, Cf remains constant as loaded initially from .sim file.
...
! look for neck cutoffs (only if enabled)
        if (flag_cutoff.eq.1) then
...
!-----------------------------------------------------------------------
        ...
        ds    = ds    * dLs**(1.d0/3.d0) * dCf**( 1.d0/3.d0) ! Apply scaling to the ds calculated from timeseries
